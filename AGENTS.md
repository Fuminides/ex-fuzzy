...
## Conformal Learning (Special Section)

The conformal module provides uncertainty-aware predictions for fuzzy classifiers. It supports:
- Calibration on a held-out dataset to guarantee coverage.
- Prediction sets (`predict_set`) for class-set outputs at a chosen miscoverage level.
- Rule-aware prediction sets (`predict_set_with_rules`) that retain explanations.
- Coverage evaluation utilities (`evaluate_conformal_coverage`).

### Recommended Workflow
1. Split data into train, calibration, and test sets.
2. Fit the base classifier on the training set.
3. Calibrate the conformal wrapper on the calibration set.
4. Predict sets on the test set and evaluate coverage.
...
### Practical Notes
- `alpha` controls the target miscoverage rate; `alpha=0.1` aims for 90% coverage.
- Calibration quality depends on the calibration set size and representativeness.
- Rule-aware outputs help trace which fuzzy rules support each class in the set.
