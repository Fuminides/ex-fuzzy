...
class classifierWrapper(ClassifierMixin):
    ...
    def __init__(self, classifier: rules.MasterRuleBase, nrule:int):
        ...
        Args:
            classifier: rule-based classifier to evaluate.
            nrule: rule to evaluate.
        """
...
def _aux_ova_rule_classifier(classifier: rules.MasterRuleBase, nrule:int, labels: np.array):
    ...
    Returns:
        an object of the sklearn classifier class.
    """
...
def permutation_p_compute(error_rates: np.array, og_error_rate: float):
    ...
    Args:
        true_values: list of true values of the test.
        error_rates: list of error rates of the test.
        og_error_rate: original error rate of the test.
...
def estimate_og_error_rate(classifier, X: np.array, labels: np.array, r:int=10):
    ...
    Returns:
        original error rate of the classifier.
    """
    og_error_rates = []
    for _ in range(r): 
...
def permutation_labels_test(classifier, X, labels, k=100, r=10):
    ...
    Args:
        ...
        r: number of repetitions to estimate the original error rate.
