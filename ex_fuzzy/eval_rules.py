...
def _shared_firing(method):
    """
    Runs an evaluation method inside the rule base's firing cache scope.

    The support, confidence, dominance and accuracy computations all ask the
    rule base for the same firing strengths. Within the scope the rule base
    computes them once and returns the same array to every request, so the
    values are exactly those of the repeated computations.
    """
    @wraps(method)
    def wrapped(self, *args, **kwargs):
        scope = getattr(self.mrule_base, '_firing_cache_scope', None)
        if scope is None:
...
class evalRuleBase():
    ...
    def __init__(self, mrule_base: rules.MasterRuleBase, X: np.array, y: np.array, time_moments: np.array=None, precomputed_truth=None) -> None:
        ...
        Args:
            mrule_base: The rule base to evaluate.
            X: array shape samples x features. The data to evaluate the rule base.
            y: array shape samples x 1. The labels of the data.
            time_moments: array shape samples x 1. The time moments of the samples. (Only for temporal rule bases)
    ...
    def compute_antecedent_pattern_support(self, X: np.array=None) -> np.array:
        ...
        if self.time_moments is None:
            antecedent_memberships = self.mrule_base.compute_firing_strengths(
    ...
    def association_degree(self) -> np.array:
        ...
        """
        if self.time_moments is None:
            firing_strengths = self.mrule_base.compute_firing_strengths(self.X)
        else:
            firing_strengths = self.mrule_base.compute_firing_strengths(self.X, self.time_moments)
    ...
    def add_classification_metrics(self, X: np.array=None, y: np.array=None) -> None:
        ...
        if self.time_moments is None:
            winning_rules, _winning_association_degrees = self.mrule_base._winning_rules(actual_X, precomputed_truth=self.precomputed_truth, allow_unknown=self.mrule_base.allow_unknown)
