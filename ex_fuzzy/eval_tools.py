...
class FuzzyEvaluator():
    ...
    def get_metric(self, metric: str, X_true: np.array, y_true: np.array, **kwargs) -> float:
        ...
        Raises:
            ValueError: If sklearn.metrics has no function with that name
            TypeError: If the keyword arguments do not fit the metric function
        ...
        Note:
            A fitted classifier predicts the labels it was trained on. When the
            classifier predicts class indexes instead (for example, one built from
            precomputed rules), named labels are converted to those indexes using
            the classifier's classes_names attribute.
        """
        # Get y predictions
        y_pred = np.asarray(self.predict(X_true))
        y_true = np.array(y_true)
        if y_true.dtype.kind in 'OUS' and y_pred.dtype.kind not in 'OUS':
        ...
        try:
            ...
            raise ValueError(f"Metric '{metric}' not found in sklearn.metrics.") from None
    ...
    def eval_fuzzy_model(self, X_train: np.array, y_train: np.array, X_test: np.array, y_test: np.array, 
        ...
        """
        # get_metric compares the labels as the classifier predicts them.
        if print_accuracy:
            print('------------')
            print('ACCURACY')
