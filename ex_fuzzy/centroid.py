...
def compute_centroid_t2_l(z: np.array, memberships: np.array) -> float:
    ...
    Args:
        z: Vector of the referencial values, in increasing order.
        memberships: vector of the fuzzy memberships.

    Returns:
        The centroid. NaN if every membership is zero.
    """
...
def compute_centroid_t2_r(z: np.array, memberships: np.array) -> float:
    ...
    Returns:
        The right component of the centroid. NaN if every membership is zero.
    """
    centros = np.mean(memberships, axis=1)
    w = centros
    if np.sum(w) == 0:
