...
def decode_rule_arrays(x: np.ndarray, n_rules: int, n_ants: int, n_features: int,
    ...
    for klass in range(n_classes):
        ...
        for source in np.flatnonzero(active & (consequents == klass)):
            ...
            # A rule is identified by its antecedents (the consequent is the
            # class here), as ``RuleSimple`` equality and hashing define it.
            # The first occurrence keeps its weight: raw bytes key the same
            # equivalence classes as the tuple, for less work per rule.
            key = row.tobytes()
            weight = float(raw_weights[source]) if ds_mode == 2 else 1.0
