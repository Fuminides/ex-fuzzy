...
def load_fuzzy_rules(rules_printed: str, fuzzy_variables: list) -> rules.MasterRuleBase:
    ...
    for line in rules_printed.splitlines():
        if line.startswith('IF'):
            ...
            if consequent == 1:
                mrule_base = rules.MasterRuleBase([_build_rule_base(fz_type, fuzzy_variables, reconstructed_rules)])
            elif consequent > 1:
                mrule_base.add_rule_base(_build_rule_base(fz_type, fuzzy_variables, reconstructed_rules))
