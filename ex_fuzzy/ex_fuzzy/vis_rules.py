...
def connect_master_rulebase(mrule_base: rules.MasterRuleBase) -> list[list[np.array]]:
    ...
    :return: List with, for each consequent, a list of pandas dataframes with the connections in adjacency matrix format. Empty for the consequents without rules.
    '''
    res = []
...
def visualize_rulebase(mrule_base: rules.MasterRuleBase, export_path: str=None) -> None:
    ...
    for ix, connected_rule_base in enumerate(connected_mrule_base):
        ...
        fig, ax = plt.subplots()
        try:
            pos = nx.nx_agraph.graphviz_layout(G_final, prog='sfdp')
        except ImportError:
        ...
        nx.draw(G_final, with_labels=True, ax=ax,
                pos=pos, node_color=node_colors)
